...
Utils.prototype.factorsOfNumber = function (number, start) {
  const FACTORS = [];
  if (number >= start) {
    // When number is even.
    if (number % 2 == 0) {
...
// 
// SET MIN AND MAX
Utils.prototype.ensureDoubleDigit = function (didgit) {
...
// HANDLE EVENT
Utils.prototype.handleEvent = function (target, event, callback) {
...
//
// HANDLE HOLDING
Utils.prototype.handleHold = function (target, callback) {
  let timeOut;
...
//
// GET HTML ELEMENT
Utils.prototype.getDom = function (attrValue) {
...
//
// SET HTML ELEMENT
Utils.prototype.setDom = function (tagName, attrObject = {}) {
...
Utils.prototype.cssRules = function (element, rules) {
  if (!element || !rules) {
...
//
// size of element
Utils.prototype.sizeOfElement = function (element) {
...
Utils.prototype.safeExistElement = function (element) {
...
//
// HAND DOM ATTR VALUE
Utils.prototype.handleDomAttrValue = function (
  method,
  element,
  attrName,
  value = ""
...
//
// SET INNERTEXT
Utils.prototype.setInnerText = function (element, text) {
  element.innerText = text;
};
//
// REMOVE ELEMENT
Utils.prototype.removeElement = function (target) {
  target.innerHTML = "";
};
//
// HANDLE PASSWORD HIDE SHOW
Utils.prototype.hideShowPassword = function (form, hideShowEle) {
...
Utils.prototype.print = function (value) {
  console.log(value);
  return value;
};
// 
// 
Utils.prototype.getJSON = function (resorce) {
...
//
// IS USER LOGGED IN
Utils.prototype.isUserLoggedIn = function (item) {
...
Utils.prototype.chartJS = function (canvaId, type, data) {
  const labels = [];
  const datas = [];
  const colors = [];
...
Utils.prototype.switchItems = function (old, obj) {
...
//
// DUPLICATES COUNTER
Utils.prototype.dups = function (array) {
  let count = {};
...
//
// SORT ARRAY
Utils.prototype.sortArr = function (array) {
...
Utils.prototype.timeToRead = function (
  text,
  wordsPerMinute = 200,
  round = true
) {
...
Utils.prototype.formatNumber = function (num, seperator) {
  return num.toString().replace(/\B(?=(\d{3})+(?!\d))/g, `${seperator}`);
};
//
// CAPTALIZE WORDS
Utils.prototype.capitalizeWords = function (str) {
  let words = str.split(" ");
...
Utils.prototype.rangedArr = function (gap, start, size) {
  let arr = [start];
...
Utils.prototype.filterValue = function (value, allowedTags = "") {
  if (value === null || value === "") {
    return null;
  }
...
//
// SET MIN AND MAX
Utils.prototype.minMax = function (min, max, value) {
  if (value > max) {
    value = min;
  } else if (value < min) {
    value = max;
...
//
// GET OBJECT PROPS
Utils.prototype.getProps = function (object, propType) {
...
//
//
// Utils.prototype.JSDSType = function (JSDS, aSet) {
//   // for (const i in JSDS) {
...
//
//
Utils.prototype.generateRandomColorScheme = function () {
...
//
//
Utils.prototype.idGenerator = function () {
...
//
//
Utils.prototype.generateSpaceColors = function () {
...
//
//
Utils.prototype.browserStorageUse = function (
  method = `post`,
  key = ``,
